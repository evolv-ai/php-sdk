...
class ActiveKeysTest extends TestCase
{
    ...
    public function shouldReturnPdpKeysWhenPredicateMached()
    {
        // Arrange
        $this->client->context->set('native.newUser', true);
        $this->client->context->set('native.pageCategory', 'pdp');

        // Act
        $activeKeys = $this->client->getActiveKeys();

        // Assert
        $this->assertEquals(['pdp.page_layout', 'pdp'], $activeKeys);
    }
    ...
    public function shouldNotReturnPageLayoutWhenPredicateNotMached()
    {
        ...
        $this->client->context->set('native.newUser', true);
        $this->client->context->set('native.pageCategory', 'pdp');
        $this->client->context->set('extra_key', true);
